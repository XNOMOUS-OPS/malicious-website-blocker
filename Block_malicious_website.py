...
# Import SIEM logging and core engine
try:
    import siem_logger
except ImportError:
    siem_logger = None
...
# Load API key from environment, .env, or fallback
...
def vt_scan_url(url):
    ...
    key = os.getenv("VT_API_KEY", VT_API_KEY)
    if not key or key == "YOUR_API_KEY":
        return {"error": "VirusTotal API key is not configured. Please set VT_API_KEY in .env or script."}
    ...
    try:
        ...
        for _ in range(5): 
            time.sleep(4)
            r = requests.get(f"https://www.virustotal.com/api/v3/analyses/{analysis_id}", headers=headers, timeout=10)
            r.raise_for_status()
            result = r.json()["data"]
...
def scan_and_display():
    ...
    messagebox.showinfo("VirusTotal Scan", "Scanning... Please wait (~15-20 seconds).")
...
def show_vt_result(url):
    ...
    # SIEM Logging
    if siem_logger:
        siem_logger.log_scan_event(url, domain, stats, is_malicious=(malicious > 0 or suspicious > 0))
    ...
    try:
        ...
        with open(hosts_path, "a", encoding="utf-8") as file:
            for domain in domains_to_block:
                ...
                if siem_logger:
                    siem_logger.log_block_event(domain, reason="GUI Manual/Auto Block", is_auto=False)
...
def unblock_websites(urls):
    ...
    try:
        ...
        for domain in domains_to_unblock:
            ...
            if siem_logger:
                siem_logger.log_unblock_event(domain, reason="GUI Manual Unblock")
...
def get_blocked_sites():
    ...
    try:
        with open(get_hosts_path(), "r", encoding="utf-8") as file:
...
app = tk.Tk()
app.title("Simple Website Blocker & SIEM Gateway") 
...
tk.Label(app, text="Currently Blocked Sites (Hosts File):").pack(pady=10)
